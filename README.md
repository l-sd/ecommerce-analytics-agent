...
GitHub Pages 在线版在浏览器内分析 4,999 行可复现模拟订单，按经营总览、渠道与转化、商品分析、用户/RFM、退款与履约、数据质量和分析报告七个页面组织。图表标注数值刻度、单位和时间顺序；分析报告会根据日期/渠道筛选更新，展示经营摘要、主要发现与口径限制。页面支持明细查看和结果下载，无需登录；数据在浏览器本地计算。在线版不支持上传自有文件；本地 Streamlit 版可上传 CSV/XLSX。
