...
一个可复现的电商订单分析作品项目：从脏数据体检、清洗和口径确认开始，自动完成核心 KPI、渠道/商品分析、RFM 用户分层，并按**流量与转化 / 销售与商品 / 用户 / 履约跟踪**四个维度展开，生成公式化 Excel、带图表的单文件 HTML 报告和验证记录；另提供本地 Streamlit 应用和浏览器端在线演示。
...
GitHub Pages 在线版默认分析 4,999 行可复现模拟订单，按经营总览、渠道与转化、商品分析、用户/RFM、退款与履约、数据质量和分析报告七个页面组织。也可上传自有 CSV/TSV 文件：至少提供订单号和金额（或单价、数量），页面自动匹配常见中英文列名，并按实际字段分析；缺少流量表、商品目录、用户 ID 或履约字段时，会说明相关分析不可用。上传数据只在浏览器本地处理，不会发送到服务器。Excel 文件请先另存为 CSV UTF-8；文件上限 20 MB。建议从[订单上传模板](docs/order-upload-template.csv)开始整理数据。多商品订单请按商品明细行提供金额，避免每行重复填写订单总额。

![仪表盘预览](docs/dashboard-preview.png)

> **在线体验：** <https://l-sd.github.io/ecommerce-analytics-agent/> —— 免登录交互式演示。
