...
GitHub Pages 在线版在浏览器内分析 4,999 行可复现模拟订单，按经营总览、渠道与转化、商品分析、用户/RFM、退款与履约、数据质量六个页面组织，支持日期/渠道筛选、明细查看和结果下载。页面无需登录；数据在浏览器本地计算。在线版不支持上传自有文件；本地 Streamlit 版可上传 CSV/XLSX。
