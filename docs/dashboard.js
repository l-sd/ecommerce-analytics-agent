(() => {
  ...
  function channelsPage(a) {
    ...
    return barPanel("流量转化漏斗",hasTraffic?funnelRows:[],num,{note:hasTraffic?"曝光、访客、加购来自配套流量表；下单数由订单明细回填。":"上传文件没有流量表，不能计算曝光、访客、加购和下单转化率。",definition:hasTraffic?"访客率 = 访客 ÷ 曝光；加购率 = 加购 ÷ 访客；下单转化率 = 下单 ÷ 访客。":"如需流量转化分析，请同时提供按日期和渠道汇总的流量表。"})+panel("渠道流量与成交对照",table(["渠道","曝光","访客","加购","下单","下单转化率","有效 GMV"],channelRows),{note:hasTraffic?"成交金额使用有效订单；流量指标来自配套流量表。":"暂无配套流量表，转化指标不可用。"})+panel("指标口径",`<div class="quality-list"><div class="quality-item"><span>流量数据来源</span><strong>${hasTraffic?"配套日 × 渠道流量表":"未提供"}</strong></div><div class="quality-item"><span>筛选口径</span><strong>日期范围与所选渠道</strong></div><div class="quality-item"><span>守恒关系</span><strong>${hasTraffic?"下单数与订单明细对齐":"无法核验"}</strong></div><div class="quality-item"><span>限制</span><strong>${datasetMeta.demo?"模拟数据，不代表真实转化水平":"仅用已上传字段，不反推流量"}</strong></div></div>`,{wide:true});
  }
  ...
  function qualityPage(a) {
    ...
    return panel("原始数据体检",`<div class="quality-list">${cells}</div>`,{wide:true,note:`质量计数基于当前数据的 ${num(q.rows)} 行原始记录；同时含单价和数量时按两者重算金额，否则使用金额列；有效订单按订单号去重。`})+panel("关键指标定义",table(["指标","计算口径","使用范围"],[["有效 GMV","有效订单金额之和","排除退款、取消和待付款状态"],["客单价","有效 GMV ÷ 有效订单数","与有效订单号数同一分母"],["退款率","退款订单 ÷ 全量订单","退款与履约页面"],["下单转化率","流量表下单数 ÷ 访客数","只使用配套流量表"],["动销率","有销量在售 SKU ÷ 在售 SKU","分母来自商品目录"],["迟发率","超时发货订单 ÷ 已发货订单","不含待发货订单"]]),{wide:true,note:"缺少对应字段时不推算或填补业务指标。"})+panel("分析校验",`<div class="quality-list"><div class="quality-item"><span>订单粒度</span><strong>金额按明细行加总，订单数按订单号去重</strong></div><div class="quality-item"><span>数据可追溯</span><strong>原始行 + 清洗后明细</strong></div><div class="quality-item"><span>流量转化来源</span><strong>${traffic.length?"配套流量表":"未提供"}</strong></div><div class="quality-item"><span>动销率分母来源</span><strong>${products.length?"商品目录":"未提供"}</strong></div></div>`,{wide:true});
  }
  function reportPage(a) {
    ...
    return panel("执行摘要",`<div class="report-scope"><span><b>日期范围</b>${safe(start)} 至 ${safe(end)}</span><span><b>所选渠道</b>${selected.length?safe(selected.join("、")):"未选择"}</span><span><b>筛选订单</b>${num(a.orderCount)} 笔</span></div><div class="report-insights">${cards}</div>`,{wide:true,note:"以下结论随上方筛选同步更新，仅描述当前文件中字段可支持的观察结果。"})+
      panel("经营指标摘要",table(["指标","当前结果","口径"],[[hasStatus?"有效 GMV":"订单金额合计",money(a.gmv),hasStatus?"排除退款、取消和待付款订单":"未提供状态，不排除退款/取消/待付款"],["有效订单",num(a.goodOrderCount),"按订单号去重"],["客单价",money(ratio(a.gmv,a.goodOrderCount)),"金额合计 ÷ 有效订单号数"],["退款率",hasStatus?pct(refundRate):"—",hasStatus?"退款订单 ÷ 全量订单":"需要订单状态列"],["复购率",hasUser?pct(repeatRate):"—",hasUser?"已知用户中复购用户占比":"需要用户标识列"]]),{note:"所有金额按当前筛选范围计算；订单数量按订单号去重。"})+
      panel("数据限制与口径",`<ul class="report-actions">${limitations.map(x=>`<li>${safe(x)}</li>`).join("")}<li>订单金额按有效明细行求和，订单数按订单号去重；请确认源表每行的金额粒度，避免重复汇总订单总额。</li><li>当前结果为描述性统计，不推断利润、ROI 或因果效果。</li></ul>`,{wide:true,note:"缺少的业务字段会明确列出，不会用模拟值补齐用户上传的数据。"});
  }
